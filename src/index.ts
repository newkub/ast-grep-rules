...
import { handleListCommand, handleScanCommand, handleFixCommand, handleCategoriesCommand } from './commands';
  ...
  try {
    ...
    cli
      ...
      .action(async (paths: string[], options: { rules?: string[]; category?: string; json?: boolean; severity?: string }) => {
        await handleScanCommand(paths, options);
      });
    ...
    cli
      ...
      .action(async (paths: string[], options: { rules?: string[]; category?: string; dryRun?: boolean }) => {
        await handleFixCommand(paths, options);
      });
